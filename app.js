...
// 家事記録 Web版 v19
// 修正内容：左側選択UIをさらに下げて中央付近へ移動、おつかい金額表示を強制的に1.2倍
...
function drawHouseworkPdfCanvas() {
  ...
  for (let i = 0; i < colWidths.length; i++) {
    ...
    drawCenteredText(ctx, headerLabels[i], x, tableY, colWidths[i], headerH, {
      fontSize: i === 2 || i === 5 ? pdfFont(15) : pdfFont(17),
      bold: true,
      lineHeight: pdfFont(18)
    });
    x += colWidths[i];
  }
  ...
  for (let rIndex = 0; rIndex < state.records.length; rIndex++) {
    ...
    drawCenteredText(ctx, record.day, cx, y, colWidths[0], rowH, { fontSize: pdfFont(18) });
    cx += colWidths[0];

    drawCenteredText(ctx, weekday(record.year, record.month, record.day), cx, y, colWidths[1], rowH, { fontSize: pdfFont(19) });
    cx += colWidths[1];
    ...
    if (hasBonus && manualBath) {
      drawCenteredText(ctx, "☆", cx, y, colWidths[2] / 2, rowH, { fontSize: pdfFont(23), bold: true });
      drawCenteredText(ctx, manualBath, cx + colWidths[2] / 2, y, colWidths[2] / 2, rowH, { fontSize: pdfFont(18) });
    } else if (hasBonus) {
      drawCenteredText(ctx, "☆", cx, y, colWidths[2], rowH, { fontSize: pdfFont(23), bold: true });
    } else {
      drawCenteredText(ctx, manualBath, cx, y, colWidths[2], rowH, { fontSize: pdfFont(18) });
    }

    cx += colWidths[2];

    drawCenteredText(ctx, circleText(record.riceCooked), cx, y, colWidths[3], rowH, { fontSize: pdfFont(26) });
    cx += colWidths[3];

    drawCenteredText(ctx, circleText(record.trashCollected), cx, y, colWidths[4], rowH, { fontSize: pdfFont(26) });
    cx += colWidths[4];

    drawCenteredText(ctx, circleText(record.vacuumed), cx, y, colWidths[5], rowH, { fontSize: pdfFont(26) });
    cx += colWidths[5];

    drawCenteredText(ctx, record.shoppingMemo || "", cx + 4, y + 2, colWidths[6] - 8, rowH / 2 - 2, { fontSize: pdfFont(13) });
    drawCenteredText(ctx, record.shoppingAmount ? yenMark(record.shoppingAmount) : "", cx + 4, y + rowH / 2, colWidths[6] - 8, rowH / 2, { fontSize: pdfFont(14) });
    cx += colWidths[6];

    drawCenteredText(ctx, record.extraMemo || "", cx + 4, y + 2, colWidths[7] - 8, rowH / 2 - 2, { fontSize: pdfFont(13) });
    drawCenteredText(ctx, minutesText(record.extraMinutes), cx + 4, y + rowH / 2, colWidths[7] - 8, rowH / 2, { fontSize: pdfFont(14) });
  }
  ...
  for (let i = 0; i < subWidths.length; i++) {
    ...
    if (i === 0) {
      drawCenteredText(ctx, subTexts[i][0], x, subtotalY, subWidths[i], subtotalH, { fontSize: pdfFont(19), bold: true });
    } else {
      ...
      drawRightText(ctx, subTexts[i][0], x, subtotalY, subWidths[i], subtotalH / 2, { fontSize: pdfFont(15), padding: 8 });
      drawRightText(ctx, subTexts[i][1], x, subtotalY + subtotalH / 2, subWidths[i], subtotalH / 2, { fontSize: pdfFont(15), padding: 8 });
    }
  ...
  drawCenteredText(ctx, "おつかい使用", shoppingBoxX, totalY, shoppingBoxW, shoppingBoxH / 2, { fontSize: pdfFont(13), bold: true });
  drawRightText(ctx, yen(summary.shoppingTotal), shoppingBoxX, totalY + shoppingBoxH / 2, shoppingBoxW, shoppingBoxH / 2, { fontSize: pdfFont(16), bold: true, padding: 10 });
  ...
  for (let i = 0; i < subWidths.length; i++) {
    ...
    drawCenteredText(ctx, unitTexts[i], x, unitY, subWidths[i], unitH, {
      fontSize: i === 5 ? pdfFont(14) : pdfFont(16),
      bold: i === 0,
      lineHeight: pdfFont(18)
    });
    x += subWidths[i];
